    ...
    with torch.no_grad():
        for phase in ['train', 'valid']:
            ...
            with torch.no_grad():
                ...
                pcnn_yhat = model_dict['pcnn_decoder'](x=target, float_condition=z_flat, spatial_condition=rec_yhat)
                ...
                for i in range(canvas.shape[1]):
                    for j in range(canvas.shape[2]):
                        ...
                        for k in range(canvas.shape[3]):
                            output = model_dict['pcnn_model_decoder'](x=canvas, float_condition=z_flat, spatial_condition=rec_yhat.detach())
